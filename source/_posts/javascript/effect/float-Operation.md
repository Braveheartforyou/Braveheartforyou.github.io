...
在用到 JavaScript 中 `float 类型`的值来运算时,会产生`精度不准`的问题。
例如：
![float operation](./float-Operation/float.jpg)
可以看到：
...
它得到的值是不精准的，简单来说，你的电脑做着正确的`二进制浮点运算`，但问题是你输入的是十进制的数，电脑以二进制运算，这两者并不是总是转化那么好的。
...
![float operation](./float-Operation/float_bug.png)
...
此时只能模仿十进制进行四舍五入了，但是二进制只有 `0 和 1` 两个，于是变为`0 舍 1 入`。这即是计算机中部分浮点数运算时`出现误差`，丢失精度的根本原因。

大整数的`精度丢失和浮点数本质上是一样`的，尾数位最大是`52`位，因此 JS 中能精准表示的最大整数是 `Math.pow(2, 53)`，十进制即 `9007199254740992`。
