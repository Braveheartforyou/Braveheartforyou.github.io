...
既然我们已经了解了如何将`颜色表示为24位序列`，那么让我们来看看如何从颜色的各个组件的值组成`颜色的24位`。假设我们有一个用`RGB（255、35、20）`表示的颜色。以下是我们如何组合这些位：
...
现在我们将目标颜色位作为`右前8位`，我们需要一种方法来屏蔽除`前8位之外`的所有其他位。这使我们回到和`（&）运算符`。请记住，`&运算符`可用于确保关闭某些位。
让我们从创建所需的位掩码开始。就像这样：
