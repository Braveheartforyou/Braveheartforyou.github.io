...
<font color="#ff502c">多路复用</font>代替原来的<font color="#ff502c">序列和阻塞机制</font>，所有就是请求的都是通过<font color="#ff502c">一个 TCP 连接并发</font>完成。同时也很好的解决了<font color="#ff502c">浏览器限制同一个域名下</font>的<font color="#ff502c">请求数量</font>的问题。
...
每一帧都包含几个字段，有**length、type、flags、stream identifier、frame playload**等，其中<font color="#ff502c">type 代表帧的类型</font>，在 HTTP/2 的标准中定义了 10 种不同的类型，包括上面所说的 HEADERS frame 和 DATA frame。此外还有：
`PRIORITY`（设置流的优先级）
`RST_STREAM`（终止流）
`SETTINGS`（设置此连接的参数）
`PUSH_PROMISE`（服务器推送）
