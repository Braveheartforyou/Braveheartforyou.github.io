...
> 注意：一个 BFC 的范围包含`创建该上下文元素的所有子元素`，但不包括`创建了新 BFC 的子元素的内部元素`。这从另一方角度说明，`一个元素不能同时存在于两个 BFC 中`。因为如果`一个元素能够同时处于两个 BFC` 中，那么就意味着这个元素能与两个 BFC 中的元素发生作用，就违反了 BFC 的隔离作用。
