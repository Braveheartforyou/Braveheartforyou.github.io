...
我们要引入`css`，并且要把`css`提出、压缩生成一个`css`文件，就要借助一个`webpack`的插件，叫做`MiniCssExtractPlugin`,他可以帮我提取css到css文件，并且压缩css。
...
> 注意，当使用`contenthash`时，如果修改js文件，css文件的hash不会变化，但是修改js的文件，css文件的hash也会变化。
